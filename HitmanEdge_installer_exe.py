...
def save_version_file(version_data=None):
    if not CONFIG_TARGET_DIR:
        return False
    version_file_path = os.path.join(CONFIG_TARGET_DIR, "version.json")
    try:
        os.makedirs(CONFIG_TARGET_DIR, exist_ok=True)
        if version_data is None:
            version_data = VERSION_INFO.copy()
        ...
        print_status(f"Version file created: v{INSTALLER_VERSION} ({BUILD_ENV})", "SUCCESS")
        return True
...
def check_existing_installation():
    ...
    if version_data:
        existing_version = version_data.get("installer_version", "0.0.0")
        existing_env = version_data.get("build_env", "unknown")
        print_status(f"Found existing installation: v{existing_version} ({existing_env})", "INFO")
...
def check_upgrade_needed(existing_version_data):
    ...
    existing_version = existing_version_data.get("installer_version", "0.0.0")
    existing_env = existing_version_data.get("build_env", "dev")

    comparison = compare_versions(INSTALLER_VERSION, existing_version)
    ...
    if CONFIG_TARGET_DIR:
        ...
        try:
            ...
            lock_data = {
                ...
                # "installation_path": CONFIG_TARGET_DIR
            }
...
def update_config_paths(config_path):
    ...
    try:
        ...
        db_name = f"hitman_edge_{env_suffix}"
        odbc_name = f"hitman_edge_{env_suffix}"
        ...
        config["app"]["VERSION"] = APPLICATION_VERSION
        config["app"]["LANGUAGE"] = "en"
        config["app"]["TIMEZONE"] = "Asia/Kolkata"
        config["version"]["INSTALLER_VERSION"] = INSTALLER_VERSION
        config["version"]["BUILD_ENV"] = BUILD_ENV
        config["version"]["APP_VERSION"] = APPLICATION_VERSION
        ...
        print_status(f"Environment: {BUILD_ENV} | DB: {db_name} | ODBC: {odbc_name}", "INFO")
        return True
    except Exception as e:
...
def download_and_extract_repo():
    try:
        ...
        # === Update config.ini in user-selected path (CONFIG_TARGET_DIR) ===
        user_config_path = dest_config
        update_config_paths(user_config_path)  # Uses CONFIG_TARGET_DIR as root
        print_status(f"Updated config.ini in user path: {user_config_path}", "SUCCESS")
       
        # === Copy only database & powerbuilder to Program Files ===
        ...
        #Update config.ini in Program Files with Program Files paths ===
        program_config_path = os.path.join(program_files_dest, "powerbuilder", "config", "config.ini")
        if os.path.exists(program_config_path):
            # Temporarily override CONFIG_TARGET_DIR to Program Files root
...
def create_system_dsn(db_info, driver_name):
    """Create a single 32-bit System DSN for the active environment."""
    print_status("Creating System DSN (32-bit)...", "INFO")
    db_name = db_info["names"][0]                 # only ONE DB
...
def is_dsn_exists(dsn_name: str) -> bool:
    """Return True if the System DSN already exists in the 32-bit registry."""
    try:
        path = r"SOFTWARE\ODBC\ODBC.INI\ODBC Data Sources"
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, path,
                             0, winreg.KEY_READ | winreg.KEY_WOW64_32KEY)
        winreg.QueryValueEx(key, dsn_name)      # raises FileNotFoundError if missing
        winreg.CloseKey(key)
        return True
    except FileNotFoundError:
        return False
...
class InstallerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title(f"Setup - HitmanEdge Application v{INSTALLER_VERSION} ({BUILD_ENV})")
...
class WelcomePage(tk.Frame):
    def __init__(self, master):
        ...
        btn_next = ttk.Button(btn_frame, text="Next >", command=master.next_page)
        btn_next.pack(side="right", padx=5)
        
        btn_cancel = ttk.Button(btn_frame, text="Cancel", command=master.quit)
...
class BrowsePage(tk.Frame):
    def __init__(self, master):
        ...
        btn_browse = ttk.Button(path_frame, text="Browse", command=self.browse)
        btn_browse.pack(side="left", padx=7, pady=0, ipady=2)

        btn_frame = tk.Frame(main_frame, bg=BG_COLOR)
        btn_frame.pack(side="bottom", fill="x", pady=10)

        btn_next = ttk.Button(btn_frame, text="Next >", command=self.proceed_to_confirmation)
        btn_next.pack(side="right", padx=5)
    ...
    def browse(self):
        folder = filedialog.askdirectory(
            initialdir=os.path.expanduser("~") if not IS_WINDOWS else r"C:\\"
        )
        if folder:
...
class ConfirmationPage(tk.Frame):
    def __init__(self, master):
        ...
        info_text = f"Installation Directory: {master.target_dir}\n\n"
        info_text += f"Build Environment: {BUILD_ENV}\n\n"
        info_text += f"Installer Version: {INSTALLER_VERSION}\n\n"
        info_text += f"Application Version: {APPLICATION_VERSION}"
        ...
        btn_confirm = ttk.Button(btn_frame, text="Confirm", command=self.confirm_installation)
    ...
    def confirm_installation(self):
        ...
        exists, existing_version = check_existing_installation()
        if exists:
            needs_upgrade, upgrade_type = check_upgrade_needed(existing_version)
            
            if upgrade_type == "same":
                response = messagebox.askyesno(
                    "Installation Already Exists",
                    f"HitmanEdge v{existing_version.get('installer_version')} ({existing_version.get('build_env')}) "
                ...
                if not response:
                    ...
                    return
            elif upgrade_type == "downgrade":
                response = messagebox.askyesno(
                    "Downgrade Warning",
                    f"You are attempting to install v{INSTALLER_VERSION} over v{existing_version.get('installer_version')}.\n\n"
                    f"This is a downgrade. Continue?",
                    icon='warning'
                )
                if not response:
                    self.master.back_page()
                ...
                response = messagebox.askyesno(
                    "Upgrade Available",
                    f"Upgrading from v{existing_version.get('installer_version')} to v{INSTALLER_VERSION}.\n\n"
                    f"A backup will be created. Continue?",
                    icon='info'
                )
                if response:
                    if not backup_existing_installation():
                        messagebox.showerror("Backup Failed", "Could not create backup. Installation aborted.")
                        return
                else:
                    self.master.back_page()
                    return
        ...
        tk.Label(
            popup,
            text=f"HitmanEdge v{INSTALLER_VERSION} ({BUILD_ENV})\n"
                 f"Installing to:{CONFIG_TARGET_DIR}\n\n"
                 f"Downloading and configuring required components",
            font=("Arial", 9),
            bg="white",
            justify="left",
            anchor="w",
...
class InstallationPage(tk.Frame):
    def __init__(self, master):
        ...
        self.install_btn = ttk.Button(btn_frame, text="Install", command=master.start_installation)
...
class FinishPage(tk.Frame):
    def __init__(self, master):
        ...
        summary_text = f"Installer Version: {INSTALLER_VERSION}\n"
        summary_text += f"Build Environment: {BUILD_ENV}\n"
        summary_text += f"Application Version: {APPLICATION_VERSION}\n"
        summary_text += f"Setup Status: {master.success_steps}/{master.total_steps} steps completed\n"
        summary_text += f"Installation Directory: {CONFIG_TARGET_DIR}\n"
        ...
        btn_finish = ttk.Button(btn_frame, text="Finish", command=self.finish_and_close)
